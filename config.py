...
def parseArgs():
    ...
    parser.add_argument("--taskSize", default=8, type=int, help = "number of image batches to load in advance")
    ...
    parser.add_argument("--calleEvery", default = 1500, type = int, help = "number of interations to call custom function after")
    ...
    parser.add_argument("--stemDropout", default=0.82, type=float, help="dropout of the Image input units")
    ...
    parser.add_argument("--wrdEmbScale", default=1.0, type=float, help="word embeddings initializtion scale")
    ...
    parser.add_argument("--initMem", default="PRM", type=str, choices=["PRM", "ZERO", "Q"], help="initializtion mod for memory")
    ...
    #parser.add_argument("--controlPostRNNmode", default="RNN", type=str) #GRU
    ...
    parser.add_argument("--readMemConcatProj", action="store_true", help="concatenate projected values instead of original to memory interaction")
...
def configCLEVR():
    config.dataPath = "{dataBasedir}/CLEVR_v1/data".format(dataBasedir = config.dataBasedir)   # not to define dataBasedir?
    config.datasetFilename = "CLEVR_{tier}_questions.json"
    config.wordVectorsFile = "./CLEVR_v1/data/glove/glove.6B.{dim}d.txt".format(dim=config.wrdEmbDim) # not to define wrdEmbDim
...
def configNLVR():
    ...
    config.imagesIdsFilename = "{tier}ImgIds.json"
